<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl text-slate-900 font-outfit leading-tight">{{ __('ai.title') }}</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($analyses->isEmpty())
                <div class="glass-panel rounded-3xl border border-slate-200/60 shadow-sm p-12 text-center max-w-md mx-auto">
                    ...
                    <h3 class="text-xl font-bold text-slate-800 font-outfit mb-2">{{ __('ai.no_analyses') }}</h3>
                    <p class="text-slate-500 text-sm mb-6">{{ __('ai.no_analyses_desc') }}</p>
                    <a href="{{ route('ai.analyses.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-primary-600 to-accent-500 hover:from-primary-500 hover:to-accent-400 text-white text-sm font-semibold rounded-xl shadow-md shadow-primary-500/15 hover:shadow-lg transition duration-150 hover:-translate-y-0.5 transform">
                        {{ __('ai.start_analysis') }}
                    </a>
                </div>
            @else
                        ...
                        @foreach($analyses as $analysis)
                            <a href="{{ route('ai.analyses.show', $analysis) }}" class="block p-4 hover:bg-slate-50/50 transition">
                                <div class="flex justify-between items-start mb-2 gap-2">
                                    <span class="text-sm font-bold text-slate-800 font-outfit">{{ __('ai.analysis_n', ['id' => $analysis->media_id]) }}</span>
                                ...
                                <div class="flex justify-between items-center text-xs text-slate-500 mt-2">
                                    <span class="font-medium text-slate-650">
                                        {{ __('common.confidence') }}: <span class="font-semibold text-slate-800">{{ $analysis->result ? round($analysis->result->confidence * 100) . '%' : '—' }}</span>
                                    </span>
                    ...
                    <div class="hidden sm:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead class="bg-slate-50/80">
                                <tr>
                                    <th class="px-6 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('ai.table_image') }}</th>
                                    <th class="px-6 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('ai.table_status') }}</th>
                                    <th class="px-6 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('ai.table_confidence') }}</th>
                                    <th class="px-6 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('ai.table_cost') }}</th>
                                    <th class="px-6 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('ai.table_time') }}</th>
                                </tr>
                            </thead>
