...
<head>
    ...
    <meta property="og:title" content="{{ __('welcome.meta_title') }}">
    <meta property="og:description" content="{{ __('welcome.meta_desc') }}">
    ...
    <header x-data="{ mobileMenuOpen: false }" class="sticky top-0 z-50 backdrop-blur-md bg-slate-950/70 border-b border-slate-900/80">
            ...
            <div class="flex justify-between h-20 items-center">
                <div class="flex items-center gap-8">
                    ...
                    <nav class="hidden md:flex items-center gap-6">
                        <a href="#features" class="text-sm text-slate-400 hover:text-white transition font-medium">{{ __('welcome.nav_features') }}</a>
                        <a href="#ai-vision" class="text-sm text-slate-400 hover:text-white transition font-medium">{{ __('welcome.nav_ai_vision') }}</a>
                        <a href="#calculator" class="text-sm text-slate-400 hover:text-white transition font-medium">{{ __('welcome.nav_calculator') }}</a>
                        <a href="#about" class="text-sm text-slate-400 hover:text-white transition font-medium">{{ __('welcome.nav_about') }}</a>
                    </nav>
                </div>
                ...
                <div class="hidden md:flex items-center gap-4">
                    @auth
                        <a href="{{ route('dashboard') }}" class="px-5 py-2.5 bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-500 hover:to-primary-600 text-white rounded-xl shadow-lg shadow-primary-600/10 text-sm font-semibold transition hover:-translate-y-0.5">{{ __('welcome.nav_dashboard') }}</a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm text-slate-300 hover:text-white transition font-medium px-4 py-2">{{ __('welcome.nav_login') }}</a>
                        <a href="{{ route('register') }}" class="px-5 py-2.5 bg-white text-slate-950 hover:bg-slate-100 rounded-xl text-sm font-bold transition hover:-translate-y-0.5 shadow-lg shadow-white/5">{{ __('welcome.nav_register') }}</a>
                    @endauth
                </div>
        ...
        <div x-show="mobileMenuOpen"
            ...
            <nav class="flex flex-col gap-3">
                <a href="#features" @click="mobileMenuOpen = false" class="text-sm text-slate-400 hover:text-white transition font-medium py-1">{{ __('welcome.nav_features') }}</a>
                <a href="#ai-vision" @click="mobileMenuOpen = false" class="text-sm text-slate-400 hover:text-white transition font-medium py-1">{{ __('welcome.nav_ai_vision') }}</a>
                <a href="#calculator" @click="mobileMenuOpen = false" class="text-sm text-slate-400 hover:text-white transition font-medium py-1">{{ __('welcome.nav_calculator') }}</a>
                <a href="#about" @click="mobileMenuOpen = false" class="text-sm text-slate-400 hover:text-white transition font-medium py-1">{{ __('welcome.nav_about') }}</a>
            </nav>
            <div class="pt-4 border-t border-slate-900 flex flex-col gap-3">
                @auth
                    <a href="{{ route('dashboard') }}" class="w-full text-center py-2.5 bg-gradient-to-r from-primary-600 to-primary-700 text-white rounded-xl text-sm font-semibold shadow-md">{{ __('welcome.nav_dashboard') }}</a>
                @else
                    <a href="{{ route('login') }}" class="w-full text-center py-2.5 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white rounded-xl text-sm font-medium">{{ __('welcome.nav_login') }}</a>
                    <a href="{{ route('register') }}" class="w-full text-center py-2.5 bg-white text-slate-950 rounded-xl text-sm font-bold shadow-md">{{ __('welcome.nav_register') }}</a>
                @endauth
            </div>
        </div>
    </header>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
                ...
                <div class="lg:col-span-7 space-y-8 text-center lg:text-left">
                    ...
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight">
                        {{ __('welcome.hero_subtitle') }}<br class="hidden sm:inline">
                        <span class="text-gradient-purple-cyan font-outfit">{{ __('welcome.hero_subtitle_2') }}</span>
                    </h1>
                    ...
                    <div class="grid grid-cols-3 gap-4 pt-8 border-t border-slate-900 max-w-lg mx-auto lg:mx-0">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 mt-1 uppercase font-semibold">{{ __('welcome.ai_accurate') }}</p>
                        </div>
                        <div>
                            <p class="text-3xl font-extrabold text-white">100%</p>
                            <p class="text-xs text-slate-500 mt-1 uppercase font-semibold">{{ __('welcome.evn_price') }}</p>
                        </div>
                        <div>
                            <p class="text-3xl font-extrabold text-white">0 VND</p>
                            <p class="text-xs text-slate-500 mt-1 uppercase font-semibold">{{ __('welcome.free_experience') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-5 relative flex justify-center">
                    <div class="relative w-full max-w-[420px] aspect-[9/16] rounded-[40px] border-[10px] border-slate-900 bg-slate-950 overflow-hidden shadow-2xl glow-primary animate-float">
                        ...
                        <div class="h-14 border-b border-slate-900 px-6 flex justify-between items-center bg-slate-950/80 backdrop-blur">
                            <span class="text-sm font-bold text-slate-200">{{ __('welcome.app_name') }}</span>
                        ...
                        <div class="p-6 space-y-5">
                            <div class="space-y-1">
                                <p class="text-xs text-slate-500">{{ __('welcome.estimated_month') }}</p>
                            ...
                            <div class="space-y-2 bg-slate-900/50 border border-slate-900 p-4 rounded-2xl">
                                <div class="flex justify-between text-xs">
                                    <span class="text-slate-400">{{ __('welcome.data_quality') }}</span>
                            ...
                            <!-- Device usage list -->
                            <div class="space-y-3">
                                <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('welcome.top_consumer') }}</p>
                                ...
                                <div class="flex justify-between items-center p-3 bg-slate-900/30 border border-slate-900/50 rounded-xl">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500">{{ __('welcome.demo_device_1') }}</p>
                                        </div>
                                    </div>
                                    <p class="text-sm font-bold text-slate-200">620,000{{ __('common.vnd_currency') }}</p>
                                </div>
                                ...
                                <div class="flex justify-between items-center p-3 bg-slate-900/30 border border-slate-900/50 rounded-xl">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500">{{ __('welcome.demo_device_2') }}</p>
                                        </div>
                                    </div>
                                    <p class="text-sm font-bold text-slate-200">285,000{{ __('common.vnd_currency') }}</p>
                                </div>
                                ...
                                <div class="flex justify-between items-center p-3 bg-slate-900/30 border border-slate-900/50 rounded-xl">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500">{{ __('welcome.demo_device_3') }}</p>
                                        </div>
                                    </div>
                                    <p class="text-sm font-bold text-slate-200">190,000{{ __('common.vnd_currency') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section id="features" class="py-20 bg-slate-950 border-t border-slate-900 relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto mb-16 space-y-4">
                <h2 class="text-3xl sm:text-4xl font-extrabold font-outfit text-white">{{ __('welcome.features_title') }}</h2>
                <p class="text-slate-400">{{ __('welcome.features_desc') }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="p-6 bg-slate-900/40 border border-slate-900 hover:border-slate-800 rounded-2xl transition duration-300 group hover:-translate-y-1">
                    ...
                    <h3 class="text-xl font-bold text-white mb-3">{{ __('welcome.feature_ai') }}</h3>
                ...
                <div class="p-6 bg-slate-900/40 border border-slate-900 hover:border-slate-800 rounded-2xl transition duration-300 group hover:-translate-y-1">
                    ...
                    <h3 class="text-xl font-bold text-white mb-3">{{ __('welcome.feature_evn') }}</h3>
                ...
                <div class="p-6 bg-slate-900/40 border border-slate-900 hover:border-slate-800 rounded-2xl transition duration-300 group hover:-translate-y-1">
                    ...
                    <h3 class="text-xl font-bold text-white mb-3">{{ __('welcome.feature_security') }}</h3>
                ...
                <div class="p-6 bg-slate-900/40 border border-slate-900 hover:border-slate-800 rounded-2xl transition duration-300 group hover:-translate-y-1">
                    ...
                    <h3 class="text-xl font-bold text-white mb-3">{{ __('welcome.feature_transparent') }}</h3>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                ...
                <div class="lg:col-span-5 space-y-6 text-center lg:text-left">
                    <h2 class="text-3xl sm:text-4xl font-extrabold font-outfit text-white">{{ __('welcome.scan_title') }}</h2>
                    <p class="text-slate-400 leading-relaxed">
                        {{ __('welcome.scan_desc') }}
                    </p>
                    ...
                    <div class="space-y-4">
                        <div class="flex gap-4 items-start text-left">
                            ...
                            <div>
                                <h4 class="font-bold text-white">{{ __('welcome.scan_approve') }}</h4>
                        ...
                        <div class="flex gap-4 items-start text-left">
                            ...
                            <div>
                                <h4 class="font-bold text-white">{{ __('welcome.smart_unit') }}</h4>
                ...
                <div class="lg:col-span-7 flex justify-center" x-data="{ scanned: false, isScanning: false, triggerScan() {
                    ...
                    <div class="w-full max-w-2xl bg-slate-900/30 border border-slate-900 p-6 sm:p-8 rounded-3xl grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                        ...
                        <div class="space-y-3 relative">
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider text-center">{{ __('welcome.label_photo') }}</p>
                        ...
                        <div class="flex flex-col justify-between">
                            <div>
                                <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{{ __('welcome.ai_result') }}</p>
                                ...
                                <div x-show="!isScanning && !scanned" class="h-48 border border-dashed border-slate-800 rounded-2xl flex flex-col items-center justify-center p-4 text-center text-slate-600">
                                    <p class="text-sm">{{ __('welcome.click_to_start') }}</p>
                                </div>
                                ...
                                <div x-show="isScanning" class="h-48 flex flex-col items-center justify-center space-y-3">
                                    ...
                                    <p class="text-sm text-slate-400 animate-pulse">{{ __('welcome.decoding') }}</p>
                                </div>
                                ...
                                <div x-show="scanned && !isScanning" class="space-y-3 transition duration-300">
                                    <div class="space-y-1">
                                        <div class="flex justify-between text-xs">
                                            <span class="text-slate-500">{{ __('welcome.device_type') }}</span>
                                            <span class="text-green-400 font-bold">{{ __('welcome.confidence_high') }}</span>
                                        </div>
                                    ...
                                    <div class="space-y-1">
                                        <div class="flex justify-between text-xs">
                                            ...
                                            <span class="text-green-400 font-bold">{{ __('welcome.confidence_med') }}</span>
                                        </div>
                                        <div class="px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-sm font-semibold text-white">
                                            HomeWatt AC-2026-X
                                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                ...
                <div class="lg:col-span-7 bg-slate-900/40 border border-slate-900 p-6 sm:p-8 rounded-3xl space-y-6 shadow-2xl">
                    <div class="flex justify-between items-center pb-4 border-b border-slate-800">
                        <h3 class="text-xl font-bold text-white">{{ __('welcome.calculator_title') }}</h3>
                        <span class="px-2.5 py-1 rounded bg-accent-500/10 text-accent-400 text-xs font-semibold">{{ __('welcome.evn_latest') }}</span>
                    </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between text-sm">
                                <span class="text-slate-400 font-medium">{{ __('welcome.rated_power') }}</span>
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between text-sm">
                                <span class="text-slate-400 font-medium">{{ __('welcome.hours_per_day') }}</span>
                                <span class="text-white font-bold" x-text="hoursPerDay + ' {{ __('common.hours_unit') }}'"></span>
                            </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between text-sm">
                                <span class="text-slate-400 font-medium">{{ __('welcome.days_per_month') }}</span>
                                <span class="text-white font-bold" x-text="daysPerMonth + ' {{ __('common.days_unit') }}'"></span>
                            </div>
                ...
                <div class="lg:col-span-5 space-y-6 text-center lg:text-left">
                    <div class="space-y-1">
                        <span class="text-xs font-bold text-accent-400 uppercase tracking-widest">{{ __('welcome.est_consumption') }}</span>
                        <h2 class="text-3xl sm:text-4xl font-extrabold font-outfit text-white">{{ __('welcome.cost_estimate') }}</h2>
                    </div>

                    <div class="p-6 bg-slate-900/20 border border-slate-900 rounded-3xl space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="text-slate-400">{{ __('welcome.energy_consumed') }}</span>
                        ...
                        <div class="flex justify-between items-center border-t border-slate-800/80 pt-4">
                            <span class="text-slate-400">{{ __('welcome.est_cost') }}</span>
                            <div class="text-right">
                                <p class="text-3xl font-extrabold text-accent-400" x-text="formatNumber(calculateCost()) + ' {{ __('common.vnd_currency') }}'"></p>
                                <p class="text-[10px] text-slate-500 font-semibold uppercase mt-0.5">{{ __('welcome.vat_note') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-3 text-sm text-slate-400 justify-center lg:justify-start">
                            <span class="text-green-400">✓</span> {{ __('welcome.feature_evn_tier') }}
                        </div>
                        <div class="flex items-center gap-3 text-sm text-slate-400 justify-center lg:justify-start">
                            <span class="text-green-400">✓</span> {{ __('welcome.feature_vat') }}
                        </div>
                        <div class="flex items-center gap-3 text-sm text-slate-400 justify-center lg:justify-start">
                            <span class="text-green-400">✓</span> {{ __('welcome.feature_formula') }}
                        </div>
                    </div>
    ...
    <section id="about" class="py-20 relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="bg-gradient-to-br from-primary-900/60 to-accent-950/60 border border-primary-500/15 p-8 sm:p-12 md:p-16 rounded-[32px] text-center space-y-6 relative overflow-hidden shadow-2xl">
                ...
                <h2 class="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-outfit text-white">{{ __('welcome.cta_title') }}</h2>
