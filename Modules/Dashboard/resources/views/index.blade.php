        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @else
                @php
                    ...
                    $activeDev = $totalDev > 0 ? $totalDev : 0;
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm hover:scale-[1.02] hover:shadow-md transition duration-200 flex flex-col justify-between h-32 relative overflow-hidden">
                        ...
                        <div class="flex items-center justify-between mt-2">
                            <span class="text-xs font-semibold text-slate-400">Tháng {{ now()->format('m/Y') }}</span>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm hover:scale-[1.02] hover:shadow-md transition duration-200 flex flex-col justify-between h-32 relative overflow-hidden">
                        <div>
                            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Thiết bị</p>
                            <h4 class="text-2xl font-extrabold text-slate-900 font-outfit mt-1.5">{{ $totalDev }}</h4>
                        </div>
                        <div class="flex items-center justify-between mt-2">
                            <span class="text-xs font-semibold text-slate-400">trong {{ $stats['total_rooms'] }} phòng</span>
                            <svg class="w-8 h-8 transform -rotate-90">
                                ...
                                <circle cx="16" cy="16" r="12" stroke="#8B5CF6" stroke-width="3.5" fill="transparent"
                                    stroke-dasharray="75.39" stroke-dashoffset="{{ $totalDev > 0 ? 75.39 * (1 - min(1, $activeDev / max($totalDev, 1))) : 75.39 }}" />
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 flex flex-col justify-between">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <span class="bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-500 px-3 py-1.5">7 ngày qua</span>
                        </div>
                        <div class="w-full">
                            ...
                            <div class="space-y-5">
                                ...
                                @else
                                    <div class="py-12 text-center">
                                        <p class="text-slate-400 text-sm">Chưa có dữ liệu tiêu thụ.</p>
                                        <p class="text-slate-400 text-xs mt-1">Thêm thiết bị và ghi nhận số đo để xem thống kê.</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <!-- Recently Added & Room Distribution -->
                    ...
                    <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            @forelse($recentDevices as $device)
                                ...
                                @endphp
                                <a href="{{ route('devices.show', $device) }}" class="border border-slate-200 bg-white rounded-2xl p-4 flex flex-col justify-between h-40 hover:border-blue-200 hover:shadow-md hover:shadow-blue-500/5 hover:-translate-y-0.5 transition duration-200">
                            ...
                            @empty
                                <div class="col-span-3 py-8 text-center text-slate-400 text-sm">Chưa có thiết bị nào. <a href="{{ route('devices.create') }}" class="text-blue-600 font-bold hover:underline">Thêm thiết bị đầu tiên</a></div>
                            @endforelse
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 flex flex-col justify-between">
                        ...
                        @if($roomsData->isEmpty() || $totalRoomKwh <= 0)
                            <div class="flex-1 flex items-center justify-center py-8">
                                <p class="text-slate-400 text-sm text-center">Chưa có dữ liệu theo phòng.<br><span class="text-xs">Dữ liệu sẽ hiển thị sau khi có số đo.</span></p>
                            </div>
                        @else
            ...
            if (ctxLine && data.length > 0) {
                ...
                new Chart(ctxLine, {
                    ...
                    data: {
                        ...
                        @if(!empty(array_filter($lastMonthData)))
                        }, {
                            label: 'Tháng trước',
                            data: lastMonthData,
                            borderColor: '#94A3B8',
