<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl text-slate-900 font-outfit leading-tight">{{ __('expense.title') }} #{{ $expense->id }} - {{ __('common.edit') }}</h2>
    </x-slot>

    @php
        $initialCategory = null;
        $oldCategoryId = old('category_id', $expense->category_id);
        if ($oldCategoryId) {
    ...
    <div class="py-12" x-data="{
        type: '{{ old('type', $expense->type) }}',
        categoryId: '{{ old('category_id', $expense->category_id) }}',
        ...
        activeTab: '{{ old('type', $expense->type) === 'income' ? 'income' : 'expense' }}',
        searchQuery: '',
        amountRaw: '{{ old('amount', (int)$expense->amount) }}',
        amountDisplay: '',
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="glass-panel rounded-2xl border border-slate-200/60 bg-white/70">
                <form method="POST" action="{{ route('expenses.update', $expense) }}" class="p-8 space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-input-label for="home_id" :value="__('expense.select_home')" />
                        <select id="home_id" name="home_id" class="mt-1 block w-full bg-slate-100 border border-slate-350 rounded-xl shadow-sm text-slate-500 py-2.5 px-3.5" disabled>
                            <option value="{{ $expense->home_id }}" selected>{{ $expense->home?->name }}</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <button type="button" @click="showCategoryModal = true" class="mt-1 flex items-center justify-between w-full bg-white/80 border border-slate-300 rounded-xl shadow-sm text-slate-855 py-2.5 px-3.5 hover:border-slate-400 transition text-left focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="occurred_at" name="occurred_at" type="datetime-local" class="mt-1 block w-full" :value="old('occurred_at', $expense->occurred_at?->format('Y-m-d\TH:i'))" required />
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100">
                        <a href="{{ route('expenses.show', $expense) }}" class="text-sm text-slate-500 hover:text-slate-800 font-semibold">{{ __('common.cancel') }}</a>
                        <x-primary-button>{{ __('expense.update_button') ?? __('Update') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all w-full max-w-lg sm:my-8"
                    ...
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between font-outfit">
                        <h3 class="text-lg font-bold text-slate-800 font-outfit">Chọn hạng mục</h3>
