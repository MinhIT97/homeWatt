                ...
                <form method="POST" action="{{ route('goal.store') }}">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="home_id" required class="w-full bg-slate-50 border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition">
                                ...
                                @foreach($homes as $home)
                                    <option value="{{ $home->id }}" @selected(old('home_id') == $home->id)>{{ $home->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required maxlength="255" placeholder="VD: Tiết kiệm mua tủ lạnh mới"
                        ...
                        <div>
                            ...
                            <select name="type" required class="w-full bg-slate-50 border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition">
                                ...
                                <option value="savings" @selected(old('type') == 'savings')>Tiết kiệm</option>
                                <option value="debt_payoff" @selected(old('type') == 'debt_payoff')>Trả nợ</option>
                                <option value="energy_reduction" @selected(old('type') == 'energy_reduction')>Giảm tiêu thụ điện</option>
                                <option value="expense_limit" @selected(old('type') == 'expense_limit')>Hạn mức chi tiêu</option>
                                <option value="income_target" @selected(old('type') == 'income_target')>Mục tiêu thu nhập</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="target_amount" value="{{ old('target_amount') }}" required min="0" step="0.01" placeholder="10000000"
                                class="w-full bg-slate-50 border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="ends_at" value="{{ old('ends_at', now()->addMonths(3)->format('Y-m-d')) }}" required
                                    class="w-full bg-slate-50 border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition">
                    ...
                    <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-slate-100">
                        <a href="{{ route('goal.index') }}" class="px-4 py-2.5 border border-slate-200 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-50 transition">Hủy</a>
                        <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-500 transition shadow-md">Tạo mục tiêu</button>
                    </div>
                </form>
            </div>
        </div>
