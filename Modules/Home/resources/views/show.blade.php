    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('homes.edit', $home) }}" class="inline-flex items-center px-4 py-2 bg-white/80 border border-slate-300 hover:border-slate-400 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 shadow-sm transition">{{ __('common.edit') }}</a>
                <a href="{{ route('homes.members', $home) }}" class="inline-flex items-center px-4 py-2 bg-white/80 border border-slate-300 hover:border-slate-400 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 shadow-sm transition">{{ __('home.member_label') }}</a>
            </div>
        </div>
    </x-slot>
            ...
            <div class="glass-panel rounded-2xl border border-slate-200/60 shadow-sm bg-white/70">
                <div class="p-6">
                    <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                        <div>
                            <dt class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('home.address_display') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-slate-800">{{ $home->address ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('home.owner_display') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-slate-800">{{ $home->owner->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('home.timezone_label') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-slate-800">{{ $home->timezone }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('home.currency_label') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-slate-800">{{ $home->currency }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('home.members_title') }}</dt>
            ...
            <div class="mt-8">
                ...
                @if($home->rooms->isEmpty())
                    <div class="glass-panel rounded-2xl border border-slate-200/60 shadow-sm bg-white/70 p-8 text-center">
                        <p class="text-slate-500 text-sm">{{ __('home.no_rooms') }}</p>
                    </div>
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($home->rooms as $room)
                            <div class="glass-panel rounded-xl border border-slate-200/60 shadow-sm p-5 bg-white/70 hover:border-primary-200 hover:-translate-y-0.5 transition duration-150 transform">
                                ...
                                <p class="text-xs text-slate-500 mt-1 capitalize">{{ __("room.types.{$room->type}") }}</p>
                            </div>
                        @endforeach
                    </div>
                @endif
