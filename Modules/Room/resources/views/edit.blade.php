<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl text-slate-900 font-outfit leading-tight">Chỉnh Sửa: {{ $room->name }}</h2>
    </x-slot>
            ...
            <div class="glass-panel rounded-2xl border border-slate-200/60 shadow-sm bg-white/70">
                <form method="POST" action="{{ route('rooms.update', $room) }}" class="p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="mt-1 block w-full bg-white/80 border border-slate-300 rounded-xl shadow-sm text-slate-800 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition duration-150 py-2.5 px-3.5">
                                @foreach(\Modules\Room\Models\Room::TYPES as $key => $label)
                                    <option value="{{ $key }}" @selected(old('type', $room->type) == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 border-t border-slate-100">
                        <a href="{{ route('rooms.show', $room) }}" class="text-sm text-slate-500 hover:text-slate-800 font-semibold transition">Hủy bỏ</a>
